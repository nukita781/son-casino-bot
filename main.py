...
def get_or_create_user(tg_id, username):
    ...
    cursor.execute('SELECT user_id, accepted_terms FROM users WHERE tg_id = ?', (tg_id,))
    result = cursor.fetchone()

    if not result:
        cursor.execute('INSERT INTO users (username, tg_id, accepted_terms) VALUES (?, ?, ?)', (username, tg_id, False))
        conn.commit()
        cursor.execute('SELECT user_id, accepted_terms FROM users WHERE tg_id = ?', (tg_id,))
        result = cursor.fetchone()
    ...
    conn.close()
    return result[0], result[1]  # user_id, accepted_terms


def mark_terms_accepted(tg_id):
    conn = sqlite3.connect('casino.db', check_same_thread=False)
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET accepted_terms = ? WHERE tg_id = ?', (True, tg_id))
    conn.commit()
    conn.close()
    logging.info(f"Пользователь {tg_id} принял соглашение")
...
def send_welcome(message):
    ...
    user_id, accepted_terms = get_or_create_user(chat_id, message.from_user.username)

    # Если пользователь уже принял соглашение - сразу показываем меню
    if accepted_terms:
        show_main_menu(message)
        return

    # Если пользователь еще не принял соглашение - показываем его
...
def handle_callback(call):
    ...
    if call.data == "decline":
        ...
        process_even_odd_game(
            get_or_create_user(chat_id, call.from_user.username)[0],
            chat_id,
            ...
            if bet_amount < 0.02:
                ...
                if user_data and user_data['balance'] >= bet_amount:
                    ...
                    if game_type == "even_odd":
                        ...
                        process_darts_game(
                            get_or_create_user(chat_id, message.from_user.username)[0],
                            chat_id,
...
if __name__ == "__main__":
    ...
    # Простая инициализация с логами
    try:
        init_db()
